...
function preflightRequest(extraHeaders = 'x-retry-count') {
  return new Request('https://example.supabase.co/functions/v1/delete-group-member', {
    method: 'OPTIONS',
    headers: {
      Origin: WEB_ORIGIN,
...
Deno.test('preflight falls back to the supabase-js header list', () => {
  const response = preflight(
    new Request('https://example.supabase.co/functions/v1/delete-group-member', { method: 'OPTIONS' })
  );
...
Deno.test('POST is not treated as preflight', () => {
  const response = preflight(
    new Request('https://example.supabase.co/functions/v1/delete-group-member', { method: 'POST' })
  );
...
Deno.test('function responses stay readable in the browser', () => {
  const response = json(
    preflightRequest(),
    { error: 'Only an admin can remove members.' },
    403
  );
